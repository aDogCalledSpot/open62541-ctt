/*  Test prepared by Nathan Pocock; nathan.pocock@opcfoundation.org
    Description: Modifies a single monitoredItem setting: TIMESTAMPS = TimestampsToReturn.Neither; 
...
Test.Execute( { Procedure: function test() {
    if( !MonitorBasicSubscription.SubscriptionCreated ) addError( "Subscription for MonitoredItemsServiceSet was not created." );
        ...
        if( CreateMonitoredItemsHelper.Execute( { SubscriptionId: MonitorBasicSubscription, ItemsToCreate: item1 } ) ) {
            ...
            item1.TimestampsToReturn = TimestampsToReturn.Neither;
            ...
            DeleteMonitoredItemsHelper.Execute( { ItemsToDelete: item1, SubscriptionId: MonitorBasicSubscription } );
        }
    }
    return( true );
} } );