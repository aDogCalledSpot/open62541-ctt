/*  Test prepared by Anand Taparia; ataparia@kepware.com
    Description: Script sets monitoring mode to 'Sampling' for an already 'Sampling' monitor item. Calls publish each time to verify that no notifications were received. */
...
function setMonitoringMode593005() {
    ...
    if( Assert.Equal( false, PublishHelper.CurrentlyContainsData(), "NotificationData received (first publish call) when none was expected for the 'Sampling' monitored item" ) ) {
        ...
        if( !uaStatus.isGood() ) {
            ...
            Assert.Equal( false, PublishHelper.CurrentlyContainsData(), "NotificationData received (second publish call) when none was expected for the 'Sampling' monitored item" );
        }
    }
...
Test.Execute( { Procedure: setMonitoringMode593005 } );