/*  Test prepared by Anand Taparia; ataparia@kepware.com
    Description: Script sets monitoring mode to 'Reporting' for a 'Sampling' monitor item. Verifies that data is received(on publish) only when the monitoring mode is 'Reporting'. */

function setMonitoringMode593006() {
    ...
    if ( !uaStatus.isGood() ) {
        ...
        if( Assert.Equal( false, PublishHelper.CurrentlyContainsData(), "NotificationData received (first publish call) when none was expected for the 'Sampling' monitored item" ) ) {
            ...
            if( !uaStatus.isGood() ) {
                ...
                addLog ( "Calling publish again. We should receive NotificationData this time." );    
                PublishHelper.Execute();

                // Make sure we received datachange notification this time
...
Test.Execute( { Procedure: setMonitoringMode593006 } );