//  Description: Example for receiving events
...
function eventsExample1() {
    ...
    */
    ...
    if( CreateMonitoredItemsHelper.Execute( { ItemsToCreate: monitoredItems, TimestampsToReturn: TimestampsToReturn.Both, SubscriptionId: MonitorEventsSubscription } ) ) {
        const MAXCOUNT = 10;
        var PublishHelper = new PublishService( { Session: Test.Session.Session } );
...
function eventsExample_WhereLIKE() {
    ...
    */
    ...
    {
        const MAXCOUNT = 10;
        var PublishHelper = new PublishService( Test.Session.Session );
        for( var publishCount=0; publishCount<MAXCOUNT; publishCount++ )
        {
            print( "Publish() call " + (1 + publishCount) + " of " + MAXCOUNT );
    ...
    if( !MonitorEventsSubscription.SubscriptionCreated ) {
        ...
        if( uaStatus.isGood() ) {
            ...
            DeleteMonitoredItemsHelper.Execute( {
				...
				SubscriptionId: MonitorEventsSubscription } );
