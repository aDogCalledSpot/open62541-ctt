    ...
    if( !MonitorBasicSubscription.SubscriptionCreated ) {
        ...
        if( CreateMonitoredItemsHelper.Execute( { ItemsToCreate: item, TimestampsToReturn: TIMES, SubscriptionId: MonitorBasicSubscription } ) ) {
            ...
            do {
                ...
                print( "Waiting " + item.RevisedSamplingInterval + "ms before publish to allowing write to commit within the Server (just in case Server responds too soon)." );
            ...
            if( PublishHelper.ReceivedDataChanges.length >= 1 ) {
                ...
                Assert.Equal( valuesToWrite[1], PublishHelper.ReceivedDataChanges[0].MonitoredItems[0].Value.Value, "Expected the first value in the DataChange queue to match the last value we wrote! Even though DiscardOldest=FALSE it makes no difference here because QueueSize=1." );
