...
function browse571Err8() {
    ...
    var maxContinuationPoints = GetMaxBrowseContinuationPoints( Test.Session.Session );
    var unlimited = false;
    if( maxContinuationPoints == 0 ) {
        addLog( "Server does not put a limit on the number of continuation points that can be allocated. Skipping test." );
        return( false );
    }
    ...
    var failurePoint = BrowseNodesUntilOutOfContinuationPoints( Test.Session.Session, nodeId, continuationPointsToTry, 0 );
    if( failurePoint < 0 ) {
...
Test.Execute( { Procedure: browse571Err8 } );