/*  Test 5.7.1-001 prepared by Nathan Pocock; nathan.pocock@opcfoundation.org
    Description: Full browse of a valid node, without filters, view=null, browseDirection = Both,
                 requestedMaxReferencesPerNode = 0, referenceTypeId = null, nodeClassMask = 0xFF, resultMask = 0x3F.
    Expected result: Service result = Good. Operation result = Good . One element in results[]; browsed references
...
function Browse_5_7_1_001() {
    ...
    if( BrowseHelper.Execute( { NodesToBrowse: m1 } ) ) {
        // all we can do here is to expect the reference count to be greater than zero.
        Assert.GreaterThan( 0, BrowseHelper.Response.Results[0].References.length, "Expected 1 or more References. Either this node is not configured with References, or, the Server did not assume a <null> ReferenceTypeId means to return all references." );
...
Test.Execute( { Procedure: Browse_5_7_1_001 } );