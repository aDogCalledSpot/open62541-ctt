...
function semantic009() {
    ...
    if( CreateMonitoredItemsHelper.Execute( { ItemsToCreate: multiStateItems[0], TimestampsToReturn: TimestampsToReturn.Server, SubscriptionId: defaultSubscription } ) ) {
        ...
        if( ltValuesModified.length > 1 ) ltValuesModified[1].Text = ltValuesModified[1].Text.substring( 0, ltValuesModified[1].Text.length - 1 );
...
Test.Execute( { Procedure: semantic009 } );