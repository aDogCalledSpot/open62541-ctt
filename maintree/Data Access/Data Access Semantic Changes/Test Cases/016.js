/*  Test prepared by nathan.pocock@opcfoundation.org
    Description: Subscribe to CubeType and change property values */

function semanticChanges016() {
    // make sure we have the necessary items configured for testing
    var result = true;
    var items = MonitoredItem.fromSettings( [ "/Server Test/NodeIds/Static/DA Profile/ArrayItemType/CubeItemType" ] );
    if( !isDefined( items ) || items.length == 0 ) {
        addSkipped( "ImageItemType is not configured in /Server Test/NodeIds/Static/DA Profile/ArrayItemType/CubeItemType. Please configure if supported." );
        return( false );
    }
    ...
    if( TranslateBrowsePathsToNodeIdsHelper.Execute( { UaBrowsePaths: browsePathsForAllItems, OperationResults: expectedResults } ) ) {
        ...
        if( ReadHelper.Execute( { NodesToRead: propertyNodes } ) ) {                   // read all of our properties
            ...
            if( CreateMonitoredItemsHelper.Execute( { ItemsToCreate: analogNodes, SubscriptionId: defaultSubscription } ) ) {
                ...
                // clean-up
                DeleteMonitoredItemsHelper.Execute( { ItemsToDelete: analogItems, SubscriptionId: defaultSubscription } );  // remove the monitored items
...
Test.Execute( { Procedure: semanticChanges016 } );