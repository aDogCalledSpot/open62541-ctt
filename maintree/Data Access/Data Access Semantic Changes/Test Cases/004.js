/*  Test prepared by Nathan Pocock; nathan.pocock@opcfoundation.org
    Description: Change the TrueState on a TwoStateDiscreteType node. */

function semantic004() {
    ...
    if( !TranslateBrowsePathsToNodeIdsHelper.Execute( { Node: twoStateItems[0], BrowsePaths: [ "TrueState" ] } ) ) {
        addError( "TwoStateDiscreteType  item '" + twoStateItems[0].NodeSetting + "' does not have a TrueState property which is NOT legal because its a MANDATORY property." );
        return( false );
    }
    var nodeTS = MonitoredItem.fromNodeIds( TranslateBrowsePathsToNodeIdsHelper.Response.Results[0].Targets[0].TargetId.NodeId )[0];
    if( !isDefined( nodeTS ) ) {
        addError( "TrueState could not be read on node: '" + twoStateItems[0].NodeSetting + "'. Aborting test." );
        return( false );
    }

    // read the engineering units, just to make sure that we can...
    if( ! ReadHelper.Execute( { NodesToRead: nodeTS } ) ) {
        addError( "Unable to read the TrueState on node: '" + twoStateItems[0].NodeSetting + "'. Aborting test." );
        return( false );
    }
    ...
    if( CreateMonitoredItemsHelper.Execute( { ItemsToCreate: twoStateItems[0], TimestampsToReturn: TimestampsToReturn.Server, SubscriptionId: defaultSubscription } ) ) {
        // now try to write to the TrueState.
        ...
        if( WriteHelper.Response.Results[0].isGood() ) {
            ...
            addSkipped( "The TrueState could not be written to " + WriteHelper.Response.Results[0] + ". Aborting test." );
...
Test.Execute( { Procedure: semantic004 } );