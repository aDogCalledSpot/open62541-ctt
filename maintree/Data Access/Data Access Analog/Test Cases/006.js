...
function write613006() {
    ...
    for( var i=0; i<analogTypeNodes.length; i++ ) {
        ...
        var currentNodeDataType = UaNodeId.GuessType( analogTypeNodes[i] ) ;
        ...
        for( var n=0; n<3; n++ ) {
            ...
            // Write the value
            if( !WriteHelper.Execute( { NodesToWrite: writeMonitoredItem } ) ) {                
            ...
            if ( ReadHelper.Execute ( { 
                ...
                addError( "Read() failed for node '" + readMonitoredItem.NodeSetting + "' with status '" + ReadHelper.Response.ResponseHeader.StatusCode + "'." );
            }
        }
    }
    return( true );
...
Test.Execute( { Procedure: write613006 } );