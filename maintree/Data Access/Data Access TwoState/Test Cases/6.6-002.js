/*  Test 6.6 Test 2; prepared by Nathan Pocock nathan.pocock@opcfoundation.org

    Description:
        Write to multiple nodes of this type, all values are True. Read back
        all of the node values.
    Expectations:
...
function write66002()
{
    ...
    if( Assert.Equal( twoStateItems.length, originalValues.length, "Incorrect value count. Aborting test." ) )
    {
        ...
        {
            twoStateItems[w].Value.Value.setBoolean( !originalValues[w] );
            print( "\tFlipped value of originalItem[" + w + "] = " + twoStateItems[w].Value.Value.toBoolean() );
        }// for w...
