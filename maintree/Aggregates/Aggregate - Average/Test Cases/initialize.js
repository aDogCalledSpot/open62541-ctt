...
if( !isDefined( CUVariables.Items.length ) || CUVariables.Items.length === 0 ) { 
    addSkipped( "No History items configured. Aborting Testing. Check settings." ); 
    stopCurrentUnit();
}
else {
    ...
    if( !Test.Connect() ) { 
        ...
        // do we have an adequate data sample?
    /*    CUVariables.Items = OPCF.HA.Analysis.RecordsetMatchesCriteria( { Items: CUVariables.Items } );
        if( CUVariables.Items.length === 0 ) { addError( "Not enough data to test against." ); stopCurrentUnit(); }*/

        // Create a reference to the aggregate function that we will use. This will help us to leverage this entire 'initialize.js' script in all other 
        // aggregates CUs and simply override the aggregate function.
        CUVariables.Aggregate = OPCF.HA.Aggregates.Averaging.Average;
    }
}