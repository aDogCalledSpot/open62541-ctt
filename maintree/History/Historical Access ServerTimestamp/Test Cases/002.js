/*  Test prepared by nathan.pocock@opcfoundation.org
    Description: Request Server timestamp only */

function servertstamp002() {
    var haparams = { 
    ...
    if( Assert.True( HistoryReadHelper.Execute( haparams ), "HistoryRead() success expected." ) ) {
        ...
        if( !Assert.Equal( new UaDateTime(), CUVariables.Items[0].Value[0].SourceTimestamp, "SourceTimestamp received even when not requested." ) ) result = false;
    } 
    else result = false;
...
Test.Execute( { Procedure: servertstamp002 } );