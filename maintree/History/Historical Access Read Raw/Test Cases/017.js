...
function readraw017() {
    var badQualityTimestamp = OPCF.HA.Analysis.Find.Date.First( { Filter: "bad", RawData: CUVariables.Items[0].RawValues } ).SourceTimestamp;
    ...
    if( Assert.True( HistoryReadHelper.Execute( haparams ), "HistoryRead() success expected." ) ) {
        ...
        if( !Assert.Equal( 3, CUVariables.Items[0].Value.length, "Wrong # records returned.", "Correct # records returned." ) ) result = false;
            ...
            if( !Assert.Equal( badQualityTimestamp,                   CUVariables.Items[0].Value[1].SourceTimestamp, "Wrong record #2 timestamp.", "Correct record #2 timestamp." ) ) result = false;
            if( !Assert.True ( CUVariables.Items[0].Value[1].StatusCode.isBad(), "Not 'BAD', record #2 StatusCode.", "Correct StatusCode (bad) record #2." ) ) result = false;
