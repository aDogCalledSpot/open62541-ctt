...
function setPublishingMode593016()
{
    ...
    if( SetPublishingModeHelper.Execute( { SubscriptionIds: [subscriptions[1], subscriptions[3]], PublishingEnabled: false } ) )
    {
        // write to the monitoredItem
        for( i=0; i<allItems.length; i++ )
        {
            GenerateScalarValue( allItems[i].Value.Value, UaNodeId.GuessType( allItems[i].NodeSetting ), 16 );
        }
        WriteHelper.Execute( {
