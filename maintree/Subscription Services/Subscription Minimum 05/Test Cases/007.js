...
function CreateSubscription017() { 
    ...
    var subs = [];
    for( var i=0; i<SUBSCRIPTIONCOUNT; i++ ) { // define the subscription
        ...
        if( !CreateSubscriptionHelper.Execute( { Subscription: subs[i] } ) ) {
            DeleteSubscriptions.Execute( { SubscriptionIds: subs } );
        }
    }//for i...

    // add a monitored item to each subscription 
    var subItems = [];
    for( var i=0; i<SUBSCRIPTIONCOUNT; i++ ) { // clone the item
        subItems.push( MonitoredItem.Clone( monitoredItem ) );
        // add to subscription
    ...
    for( var i=0; i<3; i++ ) { 
        ...
        PublishHelper.WaitInterval( { Items: subItems, Subscription:subs[0] } );
