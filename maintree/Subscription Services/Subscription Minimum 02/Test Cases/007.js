/*  Test 5.10.2 Test 20, prepared by Nathan Pocock; nathan.pocock@opcfoundation.org
    Description:
          Create two subscriptions with their priorities being equal.
          Modify the first subscriptions to have a HIGHER priority.
          Write some values and call Publish().
          Then lower the first subscriptions' priority to have the least priority.
          Write more values and call Publish(). */

function modifySubscription5102020()
{
    ...
    for( var i=0; i<2; i++ )
    {
        ...
        messages.push( "Created subscription Id: " + subscriptions[i].SubscriptionId + "; Priority: " + subscriptions[i].Priority );
    ...
    subscriptions[0].Priority += 75;
    ModifySubscriptionHelper.Execute( {
                SubscriptionId: subscriptions[0] } );
    messages.push( "Subscription id: " + subscriptions[0].SubscriptionId + " priority revised to: " + subscriptions[0].Priority );
    ...
    messages.push( "Analysis of subscription #1 as the HIGH priority subscription." );
    ...
    messages.push( "Analysis of when Subscription #1 is the LOW priority subscription." );
    messages.push( "SubscriptionId received first, most of the time: " + analysis.Values._values[0] );
    Assert.Equal( 1, analysis.Values.length(), "Expected only 1 subscription to be the dominant subscription that is returned first, most of the time." );
...
Test.Execute( { Procedure: modifySubscription5102020 } );