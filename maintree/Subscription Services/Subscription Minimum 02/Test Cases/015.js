/*  Test 5.10.3 Test 18, prepared by Nathan Pocock; nathan.pocock@opcfoundation.org
    Description: Create 2 enabled subscriptions. Verify both subscriptions are returning data. Re-enabled both subscriptions.
        Verify the subscriptions still yield dataChanges.  */

function setPublishingMode593018()
{
    var subscriptions = [ 
    new Subscription(), 
    new Subscription() ];
    ...
    if( SetPublishingModeHelper.Execute( { SubscriptionIds: subscriptions, PublishingEnabled: true } ) )
    {
        ...
        {
            GenerateScalarValue( allItems[i].Value.Value, allItems[i].DataType, 18 );
        }
        WriteHelper.Execute( {
...
Test.Execute( { Procedure: setPublishingMode593018 } );