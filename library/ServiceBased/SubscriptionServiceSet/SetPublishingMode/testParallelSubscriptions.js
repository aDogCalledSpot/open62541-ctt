...
function testParallelSubscriptions( session, subscriptionCount, nodes, mode, filter, queueLength, timestamps, requestedScanrates, subscriptionsToDisableCount )
{
    ...
    var PublishHelper = new PublishService( session );
    ...
    for( var p=0; p<MAXPUBLISHCOUNT; p++ )
    {
        ...
        if( PublishHelper.Execute() && PublishHelper.CurrentlyContainsData() )
        {
            // do we have a dataChange to look at? if not, call Publish() again!
            ackSeq = PublishHelper.CurrentDataChanges[0].SubscriptionId;
    ...
    var SetPublishingModeHelper = new SetPublishingModeService( session );
    ...
    Assert.Equal( true, SetPublishingModeHelper.Execute( { SubscriptionIds: subscriptionsToDisable, PublishingEnabled: false } ) );
    addLog( "Awaiting these subscriptions to callback: " + subscriptionsReceived.toString() );
    ...
    Assert.Equal( false, allSubscriptionsReceived, "We did not expect the disabled subscriptions to be received." );
    ...
    for( var s=0; s<subscriptionCount; s++ )
    {
        ...
        DeleteMonitoredItemsHelper.Execute( { 
			ItemsToDelete: monitoredItemsIdsToDelete, 
