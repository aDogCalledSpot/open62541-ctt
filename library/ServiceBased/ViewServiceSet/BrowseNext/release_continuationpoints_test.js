/*global UaNodeId, readSetting, GetDefaultReferencesFromNodeId, Session, addError,
  ...
  StatusCode, Assert.False
*/


function TestBrowseNextWhenMoreReferencesExist( Session, doReleaseContinuationPoints, performCleanup )
{
    ...
    var response = BrowseNextFromResponseAndAssertReferencesMatch( Session, [ expectedReferences ], 1, 1, doReleaseContinuationPoints, firstResponse.Results );
    ...
    if( !doReleaseContinuationPoints )
    {
        if( response.Results.length > 0 )
        {
            ...
            if( Assert.False( result.ContinuationPoint.isEmpty(), "ContinuationPoint is empty: " + result.ContinuationPoint ) )
            {
                response = BrowseNextFromResponseAndAssertReferencesMatch( Session, [ expectedReferences ], 2, 2, doReleaseContinuationPoints, response.Results );
            }
        }
    }
