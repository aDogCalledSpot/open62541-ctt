...
/*globals addError, Assert.Equal, Assert.NodeIdNotNull, Assert.True, BrowseDirection,
...
function TranslateBrowsePathsToNodeIdsHelperLegacy()
{
    ...
    this.validateRequest = function( expectedPathLength )
    {
        ...
        if( expectedPathLength !== undefined )
        {
            ...
            {
                if( !Assert.Equal( expectedPathLength, this.targetNames[i].length, "Test cannot be completed: wrong number of TargetNames" ) )
                {
                    return false;
                }
                if( !Assert.Equal( expectedPathLength, this.referenceTypeIds[i].length, "Test cannot be completed: wrong number of ReferenceTypeIds" ) )
                {
                    return false;
                }
            }
        ...
        for( i = 0; i < this.referenceTypeIds.length; i++ )
        {
            ...
            {
                if( !Assert.NodeIdNotNull( this.referenceTypeIds[i][j], "Test cannot be completed: referenceTypeIds[" + i + "][" + j + "] is a null NodeId" ) )
                {
                    return false;
                }
            }
    ...
    this.executeAndValidate = function( session )
    {
        ...
        {
            Assert.True( this.expectedServiceStatuses.containsExpectedStatus( this.response.ResponseHeader.ServiceResult ), "Service status is not expected: " +
