/*global UaNodeId, addError, UaTranslateBrowsePathsToNodeIdsResponse,
  Session, checkTranslateBrowsePathsToNodeIdsError, Assert.Equal,
  Assert.NodeIdsEqual, Assert.NodeIdNotNull, 
...
function TestTranslateBrowsePathsToNodeIdsMixFromRequest( Session, request, targetNodes, expectedOperationResults ) {
    var i, j;
    // call service
    ...
    if( uaStatus.isGood() ) {
        ...
        if( Assert.Equal( targetNodes.length, response.Results.length, "Wrong number of Results returned" ) ) {
            for( i = 0; i < response.Results.length; i++ ) {
                var result = response.Results[i];
                if( Assert.Equal( targetNodes[i].length, result.Targets.length, "Wrong number of Targets returned." ) ) {
                    for( j = 0; j < targetNodes[i].length; j++ ) {
                        Assert.NodeIdsEqual( targetNodes[i][j], result.Targets[j].TargetId.NodeId, "Results[" + i + "].TargetId[" + j + "] is not as expected" );
                        Assert.Equal( Math.pow( 2, 32 ) - 1, result.Targets[j].RemainingPathIndex, "Results[" + i + "].RemainingPathIndex is not as expected" );
                    }
                }
            }
        }
...
function TestTranslateBrowsePathsToNodeIdsMultiMix( Session, startingNodes, pathsToBrowse, expectedOperationResults, suppressNullTypeCheck ) {
    var i;
    ...
    if( !suppressNullTypeCheck ) {
        for( i = 0; i < referenceTypeIds.length; i++ ) {
            for( var j = 0; j < referenceTypeIds[i].length; j++ ) {
                if( !Assert.NodeIdNotNull( referenceTypeIds[i][j], "Test cannot be compelted: referenceTypeIds[" + i + "][" + j + "] is a null NodeId" ) ) {
                    return;
                }
            }
        }
    ...
    TestTranslateBrowsePathsToNodeIdsMixFromRequest( Session, request, targetNodeIds, expectedOperationResults );
}
...
function TestTranslateBrowsePathsToNodeIdsInverseMultiMix( Session, startingNodes, pathsToBrowse, expectedOperationResults, suppressNullTypeCheck ) {
    if( suppressNullTypeCheck === undefined ) {
        suppressNullTypeCheck = false;
    }
    // convert pathsToBrowse to older style
