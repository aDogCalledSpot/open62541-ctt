...
function GenerateScalarValue( value, dataType, offset ) {
    if( offset === undefined ) offset = new Date().getSeconds(); // use this to get current Second, use it to generate different values
    var temp;
    switch( dataType ) {
        case BuiltInType.Null: break;
        case BuiltInType.Boolean: value.setBoolean( offset % 2 ); break;
        case BuiltInType.Byte:    value.setByte( 129 + offset );  break;
        case BuiltInType.ByteString: 
            temp = "0x";
            for( var i=0; i<2+offset; i++ ) {
                temp += ( 16 + Math.abs( offset ) + i ).toString(16);
                if( temp.length % 2 === 1 ) temp += "0";
            }
            var bs = UaByteString.fromHexString( temp );
            value.setByteString( bs );
            break;
        case BuiltInType.DateTime:
            ...
            break;
        case BuiltInType.Double: value.setDouble( 3.1415926536 + offset ); break;
        case BuiltInType.Duration: value.setDouble( 3.1415926536 + offset ); break;
        case BuiltInType.Float:  value.setFloat( 3.1416 + offset );        break;
        case BuiltInType.Guid:
            temp = "{A48A8B6C-7C12-4C51-B830-2E436329" + ( 40960 + offset ).toString(16) + "}";
            value.setGuid( new UaGuid( temp ) );
            break;
        case BuiltInType.Int16:  value.setInt16( 2009 + offset );   break;
        case BuiltInType.Int32:  value.setInt32( 200912 + offset ); break;
        case BuiltInType.Int64:  value.setInt64( 200912021339 + offset ); break;
        case BuiltInType.NodeId: value.setNodeId( new UaNodeId() ); break;
        case DATATYPE_NUMERIC:   value.setUInt16( 60001 + offset ); break;
        case BuiltInType.QualifiedName:
            var qn = new UaQualifiedName();
            qn.Name = "uaCttqualifiedName";
            value.setQualifiedName( qn );
            break;
        case BuiltInType.SByte:  value.setSByte( 4 + offset ); break;
        case BuiltInType.String: value.setString( "UA CTT Script " + offset ); break;
        case BuiltInType.UInt16: value.setUInt16( 60001 + offset );  break;
        case BuiltInType.UInt32: value.setUInt32( 120001 + offset ); break;
        case BuiltInType.UInt64: value.setUInt64( 200912021341 + offset ); break;
        ...
        case BuiltInType.XmlElement:
            ...
            x.setString( "<uactt>script " + offset + "</uactt>" )
            value.setXmlElement( x );
            break;
        default:
            addError( "'write_attribute.js::GenerateScalarValue() - Unknown data-type; can't generate a value for: '" + dataType + "'; or " + BuiltInType.toString( dataType ) );
            break;
    }
}
...
function getMaxValueDataType ( dataType )
{
    switch ( dataType )
    {
        ...
        case BuiltInType.UInt64:
            ...
            break;
        case BuiltInType.DateTime:
            return ( UaDateTime.fromString( "9999-12-31T11:59:59.999Z" ) );
            break;
        case BuiltInType.String:
            return( WriteService.GenerateString() );
            break;
        default:
            throw ( "getMaxValueDataType() Max value requested for invalid datatype (Type Requested: " + dataType + ")." );
    }
}
...
function getMinValueDataType ( dataType )
{
    switch ( dataType )
    {
        ...
        case BuiltInType.UInt64:
            ...
            break;
        case BuiltInType.DateTime:
            return ( UaDateTime.fromString( "0001-01-01T11:12:00.000Z" ) );
            break;
        case BuiltInType.String:
            return "";
            break;
    ...
    switch ( dataType )
    {
	 ...
	 break;     
        case BuiltInType.DateTime:
            ...
            break;
        case BuiltInType.String:
            var s = WriteService.GenerateString();
            middleValue = s.substring( s.length / 2 );
            break;
        default:
...
// converts a NodeId that represents a data-type, to just the numeric representation
// that can be matched with the BuiltInType object.
function NodeIdIdentifierToDataType( nid )
{
    var bit; //built in type
    if( nid !== undefined && nid !== null && nid.IdentifierType !== undefined )
    {
        bit = nid.getIdentifierNumeric();
    }//if nid...
    return( bit );
}