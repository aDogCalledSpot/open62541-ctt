function populateNodesToWriteFromWriteMask( writeRequest, nodeIdString, writeMask )
{
    ...
    if( writeMask & AttributeWriteMask.AccessLevel )
    {
        ...
        writeRequest.NodesToWrite[currentNodeNumber].Value.Value.setByte( 31 ); //31 is the maximum value. Bits 5:7 are reserved for future use. See part 3 Table 8 for more details.
    }
    ...
    if( writeMask & AttributeWriteMask.ArrayDimensions )
    {
        ...
        writeRequest.NodesToWrite[currentNodeNumber].Value.Value = new UaVariant();
        writeRequest.NodesToWrite[currentNodeNumber].Value.Value.setUInt32( 20 );
    }
    ...
    if( writeMask & AttributeWriteMask.DataType )
    {
        ...
        var dt = new UaNodeId();
        dt.NamespaceIndex = 0;;
        writeRequest.NodesToWrite[currentNodeNumber].Value.Value.setNodeId( dt ); // 
    }
    ...
    if( writeMask & AttributeWriteMask.NodeClass )
    {
        ...
        writeRequest.NodesToWrite[currentNodeNumber].Value.Value = new UaVariant();
        writeRequest.NodesToWrite[currentNodeNumber].Value.Value.setInt16( 2 ); //Variable
    }

    if( writeMask & AttributeWriteMask.NodeId && currentNodeNumber === -1 )
    {       
        writeRequest.NodesToWrite[++currentNodeNumber].AttributeId = Attribute.NodeId;
        writeRequest.NodesToWrite[currentNodeNumber].Value.Value = new UaVariant();
        var Nid = new UaNodeId();
        Nid.namespaceIndex = 1;
        writeRequest.NodesToWrite[currentNodeNumber].Value.Value.setNodeId( Nid );
    }
    ...
    if( writeMask & AttributeWriteMask.UserWriteMask )
    {
        ...
        writeRequest.NodesToWrite[currentNodeNumber].Value.Value = new UaVariant();
        writeRequest.NodesToWrite[currentNodeNumber].Value.Value.setUInt32( 0 ); // 0 = AccessLevel
    }
    ...
    if( writeMask & AttributeWriteMask.WriteMask && currentNodeNumber === -1 )
    {
        // define a NodeId of a data-type
