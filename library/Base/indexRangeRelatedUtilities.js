...
function getWriteValues ( elementIndexStart, elementIndexEnd, dataType )
{
    ...
    switch ( dataType )
    {
        ...
        case BuiltInType.ByteString:
            ...
            {                
                ...
                for( var z=0; z<sizeOfElement; z++ ) {
                    strVal = String.fromCharCode( "A".charCodeAt( 0 ) );
                    writeValues[z] = UaByteString.fromStringData( strVal );
                }
            }
            else
            {
        ...
        case BuiltInType.LocalizedText:
            writeValues = new UaLocalizedTexts();
            for( y=0; y<sizeOfElement; y++ ) {
        ...
        writeValues = new UaQualifiedNames();
            for( y=0; y<sizeOfElement; y++ ) {
                var qn = new UaLocalizedText();
                var strVal = "";
                for( var z=0; z<sizeOfElement; z++ ) {
                    var chrVal = z+("A".charCodeAt( 0 ));
                    strVal += String.fromCharCode( chrVal );
        ...
        case BuiltInType.String:
            if( sizeOfElement == 1 )
            {
                ...
                for( y=0; y<sizeOfElement; y++ ) {
                    writeValues[y] = String.fromCharCode("A".charCodeAt(0));
                }
            }
            else
            {
        ...
        case BuiltInType.XmlElement:
            ...
            break;
        default:
            addWarning ( "Error is getWriteValues(): Unsupported datatype requested: " + BuiltInType.toString( dataType ) + " (" + dataType + ")" );
            break;
    }
...
function generateArrayWriteValue( startIndex, endIndex, dataType )
{
    ...
    switch ( dataType )
    {
        ...
        case BuiltInType.Int64:
            ...
            break;
        case BuiltInType.LocalizedText:
            value.setLocalizedTextArray( localArray );
            break;
        case BuiltInType.QualifiedName:
            value.setQualifiedNameArray( localArray );
            break;
        case BuiltInType.SByte:
        ...
        case BuiltInType.UInt64:
            ...
            break;
        case BuiltInType.Variant:
            value.setVariantArray( localArray );
            break;
        case BuiltInType.XmlElement:
            value.setXmlElementArray( localArray );
            break;
        default:
            throw ( "generateArrayWriteValue(): Unsupported datatype requested: " + BuiltInType.toString( dataType ) + " (" + dataType + ")" );
    }
    return value;
}
...
function readLastXCharacters( Subscription, itemString, dataType, numChars )
{
    try
    {
        ...
        else
        {
            ...
            {
                case BuiltInType.String:
                    writeStringToValue( Test.Session.Session, nodeIdOfItem, writeValue );
                    break;
                
                case BuiltInType.ByteString:
                    writeByteStringToValue( Test.Session.Session, nodeIdOfItem, new UaByteString( writeValue ) );
                    break;
            }
            ...
            uaStatus = Test.Session.Session.createMonitoredItems( createMonitoredItemsRequest, createMonitoredItemsResponse );
            if( uaStatus.isGood() )
            {
                addLog ( "Result:" );
                ...
                uaStatus = Test.Session.Session.publish( publishRequest, publishResponse );
...
function GetByteStringsAtIndexRange( val, ir )
{
    ...
    else
    {
        ...
        {
            // we probably only want a single value 
            if( val[dim1] !== undefined )
            {
                uabs[0] = val[dim1].getRange( dim2, dim2 );
            }
        }
        else
        {
...
function GetStringsAtIndexRange( val, ir )
{
    ...
    else
    {
        ...
        if( dim1.length !== 2 || dim2.length !== 2 )
        {
            // we probably only want specific character
            if( val[dim1] !== undefined )
            {
                uas[0] = val[dim1].substring( dim2, parseInt( dim2 ) + 1 );
            }
        }
        else
        {
            ...
            //     received "1:2,3:4"; then we care about "1" and "2" [These are the ROWS].
            // convert the dim1/2 values into numbers, they're currently strings
            dim1[0] = parseInt( dim1[0] );
            dim1[1] = parseInt( dim1[1] );
            dim2[0] = parseInt( dim2[0] );
            dim2[1] = parseInt( dim2[1] );
            var i=0;
            for( var d=dim1[0]; d<=dim1[1]; d++ )
            {
                var rawValue = val[d];
