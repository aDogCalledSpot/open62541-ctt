...
function GetMultiStateDiscreteEnumStrings( nodeSetting, sessionObject, readHelper ) {
    ...
    if( uaStatus.isGood() ) {
        ...
        for( var i=0; i<browseResponse.Results.length; i++ ) {
            ...
            Assert.True( isMultiStateDiscreteItem, "Specified node '" + item.NodeId + "' (setting: " + item.NodeSetting + ") is not of type MultiStateDiscreteType" );
        }
        if( enumStrings !== null ) {
            ...
            if( readHelper.Execute( { NodesToRead: enumStrings } ) ) {
                ...
                Assert.GreaterThan( 0, result.length, "Enumeration should have at least 1 element defined!" );
                for( var i=0; i<result.length; i++ ) {
                    print( "\t\t" + result[i].Text );
                    Assert.StringNotNullOrEmpty( result[i].Text, "Enumeration (element) should not be empty!" );
                }
            }
        }
        else {
