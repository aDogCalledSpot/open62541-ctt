...
function GetNodeIdEURange( nodeSetting ) {
    ...
    var browseRequest = GetDefaultBrowseRequest( Test.Session.Session, item.NodeId );
...
function GetEURangeAsSize( euRange ) {
    if( euRange.Low == undefined || euRange.High == undefined ) return( null );
    if( euRange.Low < 0 ) result = Math.abs( euRange.Low ) + Math.abs( euRange.High );
    else result = Math.abs( euRange.High ) - Math.abs( euRange.Low );
    return( result );
}// function GetEURangeAsSize( euRange )
...
function GetEURangeWriteValues( valuesToCreate, euRange, percentage, toPassFilter, startingNumber ) {
    ...
    if( arguments.length == 5 ) {
        ...
        var offsetSize = Math.round( GetEURangeValueFromPercent( euRange, percentage ) );
        var currentValue = startingNumber;
        var doAdd = true;
        var attempts = 100;
        if( toPassFilter ) {
            // generate values that exceed the range of the EURange percent
            while( ( results.length < valuesToCreate ) && ( attempts > 0 ) ) {
                if( doAdd ) currentValue = parseFloat( currentValue ) + parseFloat( offsetSize + 1 );
                else currentValue = parseFloat( currentValue ) - parseFloat( offsetSize + 1 );
                // prevent any OUT OF RANGE values from entering the mix
                if( currentValue < euRange.Low || currentValue > euRange.High ) currentValue = euRange.Low;
            ...
            while( ( results.length < valuesToCreate ) && ( attempts > 0 ) ) {
                ...
                if( doAdd ) currentValue = parseFloat( currentValue ) + parseFloat( incrementalValue );
                else currentValue = parseFloat( currentValue ) - parseFloat( incrementalValue );
                // prevent any OUT OF RANGE values from entering the mix
                ...
                // is the number a decimal and if so restrict to 1 decimal place
                    ...
                    if( Math.abs( startingNumber - currentValue ) >= offsetSize ) continue; 
                    results.push( currentValue );
                }
                attempts--;
            }// while...
    ...
    if( attempts === 0 ) addError( "Unable to determine values to write for testing PercentDeadband given the parameters:\n\tNumber of Values to create: " + valuesToCreate +
                                    "\n\tEuRange: " + euRange + "\n\tPercentage: " + percentage +"\n\tTo Pass Filter?: " + toPassFilter +
                                    "\n\tStarting Number: " + startingNumber );
    else print( "\tValues to " + ( toPassFilter?"Pass":"Fail" ) + " deadband filtering based on starting number '" + startingNumber + "' are:\n\t\t" + results );
    return( results );
}
