...
function repetitivelyInvoke( initializeFunction, functionToInvoke, numberOfIterations, connectOverride, disconnectOverride, timeoutOverride, testName )
{
    ...
    if( !isDefined( connectOverride ) ) {
        ...
        if( !connect( g_channel, Test.Session.Session ) ) return;
        if( !activateSession( Test.Session.Session ) ) return;
    }
    else {
        try {
            connectOverride();
    ...
    if( isDefined( disconnectOverride ) ) {
        disconnectOverride();
    }
    else {
